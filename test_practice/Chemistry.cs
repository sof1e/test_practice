...
namespace test_practice
{
    public partial class Chemistry : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (radioButton52.Checked == true) { points += 1; }
            if (radioButton46.Checked == true) { points += 1; }
            if (radioButton43.Checked == true) { points += 1; }
            if (radioButton35.Checked == true) { points += 1; }
            label8.Text = "Оцінка: " + Convert.ToString(points);
            Rating form3 = new Rating();
            form3.score_chemistry = Convert.ToString(points);
            form3.Show();
            Hide();
        }
